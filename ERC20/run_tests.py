...
class TestERC20(PyEthereumTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.s.head_state.set_balance(cls.t.a2, MAX_UINT256 * 30000)
    ...
    def test_deposit(self):
        ...
        self.assertEqual(self.c.balanceOf(self.t.a1), 0)
        self.assertIsNone(self.c.deposit(value=2000, sender=self.t.k1))
        self.assertEqual(self.c.balanceOf(self.t.a1), 2)
        # Check that 2000 Wei have been debited from a1
        self.assertEqual(initial_a1_balance - self.s.head_state.get_balance(self.t.a1), 2000)
        # ... and added to the contract
        self.assertEqual(self.s.head_state.get_balance(self.c.address), 2000)
    ...
    def test_totalSupply(self):
        ...
        self.assertEqual(self.c.totalSupply() * 1000, self.s.head_state.get_balance(self.c.address))
        self.assertTrue(self.c.withdraw(1, sender=self.t.k1))
        self.assertEqual(self.c.totalSupply(), 0)
        self.assert_tx_failed(lambda: self.c.withdraw(1, sender=self.t.k1))
        self.assertEqual(self.c.totalSupply(), 0)
    ...
    def test_transferFromAndAllowance(self):
        ...
        self.assert_tx_failed(lambda: self.c.withdraw(1, sender=self.t.k2))
        self.assertIsNone(self.c.deposit(value=1000, sender=self.t.k1))
        self.assertIsNone(self.c.deposit(value=1000, sender=self.t.k2))
        self.assertTrue(self.c.withdraw(1, sender=self.t.k1))
    ...
    def test_maxInts(self):
        initial_a1_balance = self.s.head_state.get_balance(self.t.a1)
        self.assertIsNone(self.c.deposit(value=int(MAX_UINT256), sender=self.t.k1))
        self.assertEqual(initial_a1_balance - self.s.head_state.get_balance(self.t.a1), MAX_UINT256 - 935)
    ...
    def test_failed_send_in_withdraw(self):
        ...
        self.assertIsNone(ext.my_deposit(value=2000))
        ...
        self.assertIsNone(ext2.my_deposit(value=2000))
...
class TestSolidityERC20(TestERC20):

    @classmethod
    def setUpClass(cls):
        super(TestSolidityERC20, cls).setUpClass()

        contract_code = open(PATH_TO_CONTRACTS + os.sep + 'ERC20.sol').read()
...
def load_tests(loader, tests, pattern):
    ...
    for suite in [TestSolidityERC20]:
